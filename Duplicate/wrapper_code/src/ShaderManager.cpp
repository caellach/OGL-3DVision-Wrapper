...
#endif
			if (configLine.find("[Vertex_Stereo_Custom_Params]") != std::string::npos)
			{
				...
				while (configLine.find("[End]") == std::string::npos)
				{
					...
					if (configLine.find("ShaderProgram") != std::string::npos)
					{
						...
						size_t endPoint = configLine.find(")", middlePoint1 + 1);

						shaderValue.m_programId = atoi(configLine.substr(startPoint + 1, middlePoint1 - startPoint - 1).c_str());
						shaderValue.m_convergence = (float)atof(configLine.substr(middlePoint1 + 1, middlePoint2 - middlePoint1 - 1).c_str());
						shaderValue.m_separation = (float)atof(configLine.substr(middlePoint2 + 1, endPoint - middlePoint2 - 1).c_str());
		...
		if (_inputSource.find("uniform float g_eye;\nuniform float g_eye_separation;\nuniform float g_convergence;\n") == std::string::npos)
		{
			...
			for (size_t i = 0; i < 4; i++)
			{
				...
				while (location != std::string::npos)
				{
					...
					location = newSource.find(GetVertexInjectionPoint1(), count + 1);
				}
...
///-------------------------------------------------------------------------------------------
	...
	void ShaderManager::ApplyCustomShaderPairs(GLuint progId)
	{
		for (size_t i = 0; i < m_customShaderValues.size(); i++)
		{
			if (progId == m_customShaderValues[i].m_programId)
			{
				...
				if (m_customShaderValues[i].m_separation != 0.0f)
				{
					const GLchar *uniform_eye_spearation = (GLchar*)"g_eye_separation";
					GLint location_eye_separation = (*orig_glGetUniformLocation)(progId, uniform_eye_spearation);
					(*orig_glUniform1f)(location_eye_separation, Get3DEyeSeparation() * m_customShaderValues[i].m_separation);
				}
					
				if (m_customShaderValues[i].m_convergence != 0.0f)
				{
					const GLchar *uniform_convergence = (GLchar*)"g_convergence";
					GLint location_convergence = (*orig_glGetUniformLocation)(progId, uniform_convergence);
					(*orig_glUniform1f)(location_convergence, Get3DConvergence() * m_customShaderValues[i].m_convergence);
				}
