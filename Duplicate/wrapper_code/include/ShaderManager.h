...
public:
	...
	void ApplyCustomShaderPairs(GLuint progId);
	...
	// Set current Active Shader PRogram ID
		inline void SetActiveShaderProgram(GLuint _currentProgram)
	{
		m_currentProgramId = _currentProgram;
	}

	// Get current Active Shader PRogram ID
	inline GLuint GetActiveShaderProgram()
	{
